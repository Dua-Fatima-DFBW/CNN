...
$sql = "SELECT DISTINCT users.user_id, users.username, users.profile_image, users.status 
        FROM users 
        ...
        AND (users.username LIKE '%{$searchTerm}%' OR users.phone LIKE '%{$searchTerm}%')";
...
if (mysqli_num_rows($query) > 0) {
    while ($row = mysqli_fetch_assoc($query)) {
        ...
        $output .= '<div class="contact-item" onclick="openChat(' . $row['user_id'] . ', \'' . $row['username'] . '\', \'uploads/' . $row['profile_image'] . '\', \'' . $row['status'] . '\')">
                        <img src="uploads/' . $row['profile_image'] . '" alt="" class="profile-img">
                        <div class="contact-info">
                            <div class="contact-name-row">
                                <span class="contact-name">' . $row['username'] . '</span>
    ...
    $output .= '<div style="padding: 20px; text-align: center; color: var(--text-secondary);">No contacts found matching "' . $searchTerm . '"</div>';
}
echo $output;
?>
